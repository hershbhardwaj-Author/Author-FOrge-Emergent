...
const DECISIONS = {
  Authority: {
    without: {
      title: "You're the best-kept secret in your industry",
      bullets: [
    ...
    with: {
      title: "You're the recognised authority of your field",
      bullets: [
        "Your book opens the conversation for you",
        "Prospects arrive already convinced",
        "You become the named voice of the category",
        "Permanent third-party validation in print",
      ],
    },
  },
  Income: {
    without: {
      title: "You're trading hours for fees",
      bullets: [
        "Discovery calls without anchored authority",
        "Pricing pressure on every proposal",
        "Income tied directly to capacity",
        "No leverage beyond personal output",
      ],
    },
    with: {
      title: "Premium pricing becomes the default",
      bullets: [
        "Book buyers self-qualify into high-ticket offers",
        "Fees expand because the author commands them",
        "Speaking, advisory and licensing open up",
        "Revenue compounds beyond your hours",
      ],
    },
  },
  Speaking: {
    without: {
      title: "You chase rooms that don't return the call",
      bullets: [
        "No proof-of-thought asset to send",
        "Bureau lists overlook you",
        "Local events instead of category stages",
        "Fees are quoted, never set",
      ],
    },
    with: {
      title: "Stages and bureaus come to you",
      bullets: [
        "The book becomes your speaker reel",
        "Bureaus list you with a named title",
        "Keynotes replace breakouts",
        "Fees follow the asset, not the negotiation",
      ],
    },
  },
  Legacy: {
    without: {
      title: "Your work lives only in your calendar",
      bullets: [
        "Insights stay inside private decks",
        "Your IP is locked to client work",
        "The next generation never finds you",
        "Your name fades when the calendar empties",
      ],
    },
    with: {
      title: "Your thinking outlives every cohort",
      bullets: [
        "A permanent record of your method",
        "An asset that continues to teach",
        "An estate of intellectual property",
        "A name that compounds for decades",
      ],
    },
  },
};
...
const DecisionSection = () => {
  ...
  const data = DECISIONS[active];
  const nextIdx = (labels.indexOf(active) + 1) % labels.length;
    ...
    <section data-testid="section-comparison" className="bg-[var(--ia-ivory-warm)] border-b hairline">
      <div className="max-w-[1480px] mx-auto px-6 sm:px-12 lg:px-20 py-20 sm:py-28">
        <div className="text-center max-w-2xl mx-auto mb-16">
          ...
          <p className="mt-6 text-[var(--ia-ink-mute)] text-lg italic">
            Pick a dimension. See exactly what changes when you publish your book through The Forge.
          </p>
        </div>

        <div className="grid grid-cols-4 border-y hairline">
          {labels.map((label) => (
            <button
              key={label}
              data-testid={`tab-${label.toLowerCase()}`}
              onClick={() => setActive(label)}
              className={`tab-underline py-5 sm:py-7 px-2 sm:px-4 font-display text-[18px] sm:text-2xl tracking-tight ${active === label ? "is-active text-[var(--ia-ink)]" : "text-[var(--ia-ink-mute)]"}`}
            >
              {label}
            </button>
          ))}
        ...
        <div key={active} className="grid grid-cols-1 lg:grid-cols-2 min-h-[440px]">
          <div className="lg:border-r hairline p-10 lg:p-16 flex flex-col block-fade">
          ...
          <div className="p-10 lg:p-16 flex flex-col bg-[var(--ia-ivory-deep)] block-fade">
            <div className="flex items-center gap-3 mb-8">
              <span className="w-7 h-7 border border-[var(--ia-forest)] rounded-full flex items-center justify-center text-[var(--ia-forest)]">
                <Check size={12} strokeWidth={1.5} />
              </span>
              <Eyebrow className="text-[var(--ia-forest)]">With The Author's Forge</Eyebrow>
            </div>

            {!isRevealed ? (
              <div className="flex-1 flex flex-col items-start justify-center gap-8">
                <div className="font-display-italic text-[42px] sm:text-[56px] leading-[0.95] text-[var(--ia-ink)]/40">
                  Reveal the after
                </div>
                <button
        ...
        <div className="border-t hairline pt-6 flex items-center justify-between">
          <button
            ...
            onClick={() => setActive(labels[nextIdx])}
            className="btn-ghost"
          >
          ...
          <div className="flex gap-3">
            {labels.map((l) => (
              <button
                ...
                aria-label={l}
                onClick={() => setActive(l)}
                className={`w-2 h-2 rounded-full transition-colors ${active === l ? "bg-[var(--ia-ink)]" : "bg-[var(--ia-rule)]"}`}
              />
            ))}
          </div>
        </div>
